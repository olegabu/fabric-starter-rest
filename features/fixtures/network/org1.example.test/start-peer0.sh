...
main() {
    ...
    docker volume rm ordererexampletest_orderer peer0org1exampletest_ledger peer0org1exampletest_admin_app peer0org1exampletest_nginx_templates

    sleep 1
    truncate --size 0 peer0.org1.example.test/crypto-config/hosts
    truncate --size 0 peer2.org1.example.test/crypto-config/hosts

    pushd peer2.org1.example.test
        gitCleanDir crypto-config
    popd

    pushd peer0.org1.example.test
        gitCleanDir ledger
        gitCleanDir crypto-config
